import { useState } from 'react';

const sizeGroups = {
  ropa: ['XS', 'S', 'M', 'L', 'XL', 'XXL'],
  zapatos: ['35', '36', '37', '38', '39', '40'],
  pantalones: ['25', '26', '27', '28', '29', '30', '32'],
};

export default function ProductDetail({ onBack }) {
  const [name, setName] = useState('Vestido Floral Verano');
  const [cat, setCat] = useState('Vestidos');
  const [price, setPrice] = useState('89900');
  const [status, setStatus] = useState('Activo');
  const [selectedSizes, setSelectedSizes] = useState(['XS', 'S', 'M', 'L']);
  const [colors, setColors] = useState(['Rosa', 'Azul']);
  const [newColor, setNewColor] = useState('');
  const [saved, setSaved] = useState(false);
  const [tab, setTab] = useState('general');

  function toggleSize(s) {
    setSelectedSizes((prev) => (prev.includes(s) ? prev.filter((x) => x !== s) : [...prev, s]));
  }
  function addColor() {
    if (newColor.trim()) {
      setColors((prev) => [...prev, newColor.trim()]);
      setNewColor('');
    }
  }
  function save() {
    setSaved(true);
    setTimeout(() => setSaved(false), 2500);
  }

  const tabs = [
    { id: 'general', label: 'Información general' },
    { id: 'inventario', label: 'Inventario por talla' },
    { id: 'imagenes', label: 'Imágenes' },
  ];

  return (
    <div className="p-8 max-w-4xl">
      <button
        onClick={onBack}
        className="flex items-center gap-2 text-sm mb-6 font-medium transition-colors"
        style={{ color: '#81638b' }}
      >
        ← Volver a productos
      </button>

      {/* Tabs */}
      <div className="flex gap-1 mb-6 p-1 rounded-xl w-fit" style={{ background: '#f5f0f7' }}>
        {tabs.map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className="px-4 py-2 rounded-lg text-sm font-semibold transition-all"
            style={{
              background: tab === t.id ? '#ffffff' : 'transparent',
              color: tab === t.id ? '#503459' : '#81638b',
              boxShadow: tab === t.id ? '0 1px 3px rgba(80,52,89,0.1)' : 'none',
            }}
          >
            {t.label}
          </button>
        ))}
      </div>

      {tab === 'general' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {/* Main form */}
          <div className="md:col-span-2 space-y-4">
            <div className="bg-white rounded-2xl p-6 border" style={{ borderColor: '#e8dff0' }}>
              <h3 className="text-sm font-semibold mb-4" style={{ color: '#503459' }}>
                Datos del producto
              </h3>
              <div className="space-y-4">
                <div>
                  <label
                    className="block text-xs font-semibold mb-1.5 uppercase tracking-wide"
                    style={{ color: '#503459' }}
                  >
                    Nombre
                  </label>
                  <input
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    className="w-full px-4 py-2.5 rounded-xl border text-sm outline-none"
                    style={{ borderColor: '#dac9df', color: '#503459' }}
                    onFocus={(e) => (e.currentTarget.style.borderColor = '#81638b')}
                    onBlur={(e) => (e.currentTarget.style.borderColor = '#dac9df')}
                  />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label
                      className="block text-xs font-semibold mb-1.5 uppercase tracking-wide"
                      style={{ color: '#503459' }}
                    >
                      Categoría
                    </label>
                    <select
                      value={cat}
                      onChange={(e) => setCat(e.target.value)}
                      className="w-full px-4 py-2.5 rounded-xl border text-sm outline-none"
                      style={{ borderColor: '#dac9df', color: '#503459' }}
                    >
                      {['Vestidos', 'Blusas', 'Pantalones', 'Faldas', 'Conjuntos', 'Abrigos', 'Tops', 'Accesorios'].map(
                        (c) => (
                          <option key={c}>{c}</option>
                        ),
                      )}
                    </select>
                  </div>
                  <div>
                    <label
                      className="block text-xs font-semibold mb-1.5 uppercase tracking-wide"
                      style={{ color: '#503459' }}
                    >
                      Proveedor
                    </label>
                    <select
                      className="w-full px-4 py-2.5 rounded-xl border text-sm outline-none"
                      style={{ borderColor: '#dac9df', color: '#503459' }}
                    >
                      {['Textiles Bogotá', 'ModaCali S.A.', 'DenimCo', 'LuxFashion', 'KnitCo'].map((p) => (
                        <option key={p}>{p}</option>
                      ))}
                    </select>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label
                      className="block text-xs font-semibold mb-1.5 uppercase tracking-wide"
                      style={{ color: '#503459' }}
                    >
                      Precio de venta
                    </label>
                    <div className="relative">
                      <span className="absolute left-3 top-2.5 text-sm" style={{ color: '#b695c0' }}>
                        $
                      </span>
                      <input
                        value={price}
                        onChange={(e) => setPrice(e.target.value)}
                        className="w-full pl-7 pr-4 py-2.5 rounded-xl border text-sm outline-none font-mono"
                        style={{ borderColor: '#dac9df', color: '#503459' }}
                      />
                    </div>
                  </div>
                  <div>
                    <label
                      className="block text-xs font-semibold mb-1.5 uppercase tracking-wide"
                      style={{ color: '#503459' }}
                    >
                      Precio de costo
                    </label>
                    <div className="relative">
                      <span className="absolute left-3 top-2.5 text-sm" style={{ color: '#b695c0' }}>
                        $
                      </span>
                      <input
                        defaultValue="45000"
                        className="w-full pl-7 pr-4 py-2.5 rounded-xl border text-sm outline-none font-mono"
                        style={{ borderColor: '#dac9df', color: '#503459' }}
                      />
                    </div>
                  </div>
                </div>
                <div>
                  <label
                    className="block text-xs font-semibold mb-1.5 uppercase tracking-wide"
                    style={{ color: '#503459' }}
                  >
                    Descripción
                  </label>
                  <textarea
                    rows={3}
                    defaultValue="Vestido floral de verano en tela fresca y liviana. Ideal para ocasiones casuales y de playa."
                    className="w-full px-4 py-2.5 rounded-xl border text-sm outline-none resize-none"
                    style={{ borderColor: '#dac9df', color: '#503459' }}
                    onFocus={(e) => (e.currentTarget.style.borderColor = '#81638b')}
                    onBlur={(e) => (e.currentTarget.style.borderColor = '#dac9df')}
                  />
                </div>
              </div>
            </div>

            {/* Tallas */}
            <div className="bg-white rounded-2xl p-6 border" style={{ borderColor: '#e8dff0' }}>
              <h3 className="text-sm font-semibold mb-4" style={{ color: '#503459' }}>
                Tallas disponibles
              </h3>
              <div className="flex gap-2 flex-wrap">
                {sizeGroups.ropa.map((s) => (
                  <button
                    key={s}
                    onClick={() => toggleSize(s)}
                    className="w-12 h-10 rounded-xl text-sm font-semibold border-2 transition-all"
                    style={{
                      background: selectedSizes.includes(s) ? '#81638b' : '#fff',
                      borderColor: selectedSizes.includes(s) ? '#81638b' : '#dac9df',
                      color: selectedSizes.includes(s) ? '#fff' : '#81638b',
                    }}
                  >
                    {s}
                  </button>
                ))}
              </div>
            </div>

            {/* Colores */}
            <div className="bg-white rounded-2xl p-6 border" style={{ borderColor: '#e8dff0' }}>
              <h3 className="text-sm font-semibold mb-4" style={{ color: '#503459' }}>
                Colores
              </h3>
              <div className="flex flex-wrap gap-2 mb-3">
                {colors.map((c) => (
                  <span
                    key={c}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium"
                    style={{ background: '#dac9df', color: '#503459' }}
                  >
                    {c}
                    <button
                      onClick={() => setColors((prev) => prev.filter((x) => x !== c))}
                      className="ml-1 text-xs"
                      style={{ color: '#81638b' }}
                    >
                      ×
                    </button>
                  </span>
                ))}
              </div>
              <div className="flex gap-2">
                <input
                  value={newColor}
                  onChange={(e) => setNewColor(e.target.value)}
                  onKeyDown={(e) => e.key === 'Enter' && addColor()}
                  placeholder="Agregar color..."
                  className="flex-1 px-3 py-2 rounded-xl border text-sm outline-none"
                  style={{ borderColor: '#dac9df', color: '#503459' }}
                />
                <button
                  onClick={addColor}
                  className="px-4 py-2 rounded-xl text-sm font-semibold"
                  style={{ background: '#dac9df', color: '#503459' }}
                >
                  + Agregar
                </button>
              </div>
            </div>
          </div>

          {/* Sidebar */}
          <div className="space-y-4">
            <div className="bg-white rounded-2xl p-5 border" style={{ borderColor: '#e8dff0' }}>
              <h3 className="text-sm font-semibold mb-4" style={{ color: '#503459' }}>
                Estado
              </h3>
              <div className="flex gap-2">
                {['Activo', 'Inactivo'].map((s) => (
                  <button
                    key={s}
                    onClick={() => setStatus(s)}
                    className="flex-1 py-2 rounded-xl text-sm font-semibold border-2 transition-all"
                    style={{
                      background: status === s ? '#81638b' : '#fff',
                      borderColor: status === s ? '#81638b' : '#dac9df',
                      color: status === s ? '#fff' : '#81638b',
                    }}
                  >
                    {s}
                  </button>
                ))}
              </div>
            </div>

            <div className="bg-white rounded-2xl p-5 border" style={{ borderColor: '#e8dff0' }}>
              <h3 className="text-sm font-semibold mb-4" style={{ color: '#503459' }}>
                Colección
              </h3>
              <select
                className="w-full px-3 py-2.5 rounded-xl border text-sm outline-none"
                style={{ borderColor: '#dac9df', color: '#503459' }}
              >
                <option>Temporada Primavera 2026</option>
                <option>Colección Noche</option>
                <option>Básicos</option>
                <option>Edición Limitada</option>
              </select>
            </div>

            <div className="rounded-2xl p-5 border" style={{ background: '#dac9df', borderColor: '#b695c0' }}>
              <p className="text-xs font-semibold mb-2 uppercase tracking-wide" style={{ color: '#503459' }}>
                Margen de ganancia
              </p>
              <p className="text-3xl font-bold" style={{ color: '#503459' }}>
                50%
              </p>
              <p className="text-xs mt-1" style={{ color: '#81638b' }}>
                Costo: $45.000 → Precio: $89.900
              </p>
            </div>

            <button
              onClick={save}
              className="w-full py-3 rounded-xl text-sm font-semibold text-white transition-all"
              style={{ background: saved ? '#2d6a4f' : '#81638b' }}
            >
              {saved ? '✓ Guardado' : 'Guardar cambios'}
            </button>
            <button
              className="w-full py-2.5 rounded-xl text-sm font-semibold border transition-all"
              style={{ borderColor: '#dac9df', color: '#81638b' }}
            >
              Cancelar
            </button>
          </div>
        </div>
      )}

      {tab === 'inventario' && (
        <div className="bg-white rounded-2xl border overflow-hidden" style={{ borderColor: '#e8dff0' }}>
          <table className="w-full text-sm">
            <thead>
              <tr style={{ background: '#f5f0f7' }}>
                {['Talla', 'Color', 'Stock actual', 'Stock mínimo', 'Precio especial', 'Acciones'].map((h) => (
                  <th
                    key={h}
                    className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide"
                    style={{ color: '#81638b' }}
                  >
                    {h}
                  </th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y" style={{ borderColor: '#f5f0f7' }}>
              {selectedSizes
                .flatMap((s) => colors.map((c) => ({ s, c, stock: Math.floor(Math.random() * 15) })))
                .map((row) => (
                  <tr key={row.s + row.c}>
                    <td className="px-5 py-3">
                      <span className="font-semibold" style={{ color: '#503459' }}>
                        {row.s}
                      </span>
                    </td>
                    <td className="px-5 py-3 text-xs" style={{ color: '#81638b' }}>
                      {row.c}
                    </td>
                    <td className="px-5 py-3">
                      <input
                        defaultValue={row.stock}
                        type="number"
                        className="w-20 px-3 py-1.5 rounded-lg border text-sm outline-none text-center font-mono"
                        style={{ borderColor: '#dac9df', color: '#503459' }}
                      />
                    </td>
                    <td className="px-5 py-3">
                      <input
                        defaultValue={5}
                        type="number"
                        className="w-20 px-3 py-1.5 rounded-lg border text-sm outline-none text-center"
                        style={{ borderColor: '#dac9df', color: '#503459' }}
                      />
                    </td>
                    <td className="px-5 py-3">
                      <input
                        placeholder="—"
                        className="w-28 px-3 py-1.5 rounded-lg border text-sm outline-none font-mono"
                        style={{ borderColor: '#dac9df', color: '#503459' }}
                      />
                    </td>
                    <td className="px-5 py-3">
                      <button
                        className="text-xs px-2 py-1 rounded-lg"
                        style={{ background: '#dac9df', color: '#503459' }}
                      >
                        Ajustar
                      </button>
                    </td>
                  </tr>
                ))}
            </tbody>
          </table>
        </div>
      )}

      {tab === 'imagenes' && (
        <div className="grid grid-cols-3 gap-4">
          <img
            src="https://images.unsplash.com/photo-1572804013309-59a88b7e92f1?w=300&h=300&fit=crop&auto=format"
            alt="Producto 1"
            className="rounded-2xl object-cover w-full aspect-square"
          />
          <img
            src="https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=300&h=300&fit=crop&auto=format"
            alt="Producto 2"
            className="rounded-2xl object-cover w-full aspect-square"
          />
          <div
            className="rounded-2xl aspect-square flex flex-col items-center justify-center gap-2 border-2 border-dashed cursor-pointer transition-all"
            style={{ borderColor: '#b695c0' }}
          >
            <span className="text-3xl" style={{ color: '#b695c0' }}>
              +
            </span>
            <span className="text-xs font-medium" style={{ color: '#b695c0' }}>
              Agregar imagen
            </span>
          </div>
        </div>
      )}
    </div>
  );
}
