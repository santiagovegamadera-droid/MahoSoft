...
- `src/features/<feature>/` - One folder per domain (`auth`, `dashboard`, `products`, `categories`, `inventory`, `pos`, `customers`, `suppliers`, `users`, `reports`). Screens, and any components/hooks/data used only by that feature, live here.
